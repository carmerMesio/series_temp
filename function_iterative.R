...

{
  ...
  for(i in nrow(datos):363){
    ...
    for(t in 1:ncol(serief1ts)){
      vararima <- auto.arima(serief1ts[,t],max.p = 5,max.q = 2,stationary = TRUE,xreg = cbind(dat=datdom2,fest=fest2,nad=nad2))
...
autoplay = FALSE, 
interval = 0.5, imgdir = "animate_img", htmlfile = "algorithm_noexo.html", 
